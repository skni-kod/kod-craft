...
double Block::checkCollision(EntityPosition position, EntityPosition velocity, Hitbox* hitbox, WorldPos x, WorldPos y, WorldPos z) {
    for (int i = 0; i < this->propeties->hitboxes.size(); i++) {
        double delta = hitbox->collideWithBlock(position, velocity, this->propeties->hitboxes[i], x, y, z);
        if (delta == 0) continue;
        return delta;
    }
